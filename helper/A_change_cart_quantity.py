...
class Sign_Up(DDT_edge):
    def check_if_success(self):
        while True:
            try:
                is_success = True
                style = self.find_ele(
                    By.ID, 'signupModal').get_attribute('class')
                if style == 'modal fade':
                    warn = self.find_ele(
                        By.XPATH, '/html/body/div[3]').get_attribute('class')
                    if warn == 'alert alert-warning alert-dismissible fade show':
                        is_success = False
                else:
                    is_success = False
                    close_btn = self.find_ele(
                        By.XPATH, '//*[@id="signupModal"]/div/div/div[1]/button')
    ...
    def sign_up(self, record):
        ...
        temp = self.find_ele(
            By.XPATH, '//*[@id="signupModal"]/div/div/div[2]/form/button')
        self.click(temp)
