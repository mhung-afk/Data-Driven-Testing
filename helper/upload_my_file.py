...
class Upload_My_File_DDT_edge(DDT_edge):
    ...
    def remove_file(self, name):
        ...
        while True:
            try:
                # dele = self.find_ele(
                #     By.XPATH, '//button[@class="fp-file-delete btn btn-secondary"]')
                dele = self.find_ele(
                    By.XPATH, '//div[@class="form-group mx-0"]/button[2]')
        ...
        self.wait(1)
        acc = self.find_ele(
            By.XPATH, '//button[@class="fp-dlg-butconfirm btn-primary btn"]')
        self.click(acc)

    def upload_a_file(self, name):
        if str(name) == 'N':
            return

        while True:
            try:
                area = self.find_ele(By.CSS_SELECTOR, '.filemanager-container')
                self.wait(1)
                self.drop_files(area, os.getcwd()+'/test-data/'+str(name))
                break
            except:
                self.wait(1)

    def upload_my_file(self, record):

        self.upload_a_file(record[1])
        self.wait(1)

        self.upload_a_file(record[2])
        self.wait(1)

        self.remove_file(record[3])
        self.wait(1)

        self.upload_a_file(record[4])
        self.wait(1)
